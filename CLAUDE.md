...
- "Fix a mysterious failure" → "Confirm the actual cause with a minimal isolating probe *before* applying a fix" — a plausible theory can be wrong and cost a full cycle (e.g. renaming `node_modules` on the theory that Netlify skips it, when a marker-file probe showed the real cause was dot-directories like `.pnpm`). When a bug class has already been "fixed" multiple times and keeps recurring (three prior PRs for "share from YouTube fails" — sqlite preflight, then Shorts MIME handling, then MIME handling again — each guessed from vague user feedback with no stack trace), check whether the evidence actually supports a *new* theory before shipping a fourth blind patch: it turned out the in-app diagnostics attached to those reports never carried anything about the share itself, because the log buffer is in-memory and resets before a user reopens the app to file feedback — so every prior "fix" was a guess with no way to confirm it addressed the real failure. The correct move was to add durable instrumentation and wait for real evidence, not to guess again.
...
- "Replace a stateful animation/hysteresis primitive (`diffClamp`, a debounce, a clamp) with a custom reducer" → **first write down the invariant the primitive actually guarantees, then design directly to that invariant — don't patch a naive model one reviewer-found sequence at a time.** `diffClamp`'s real invariant is "track the running min/max of the signal since the last flip, not the value at the flip itself" (peak/trough tracking). `domain/header-collapse.ts` (STASH-2G, PR #504) skipped that step and went through three review rounds on the same ~15-line function: v1 compared only the two latest scroll readings (regressed — a multi-tick gesture flip-flopped every tick); v2 re-stamped an anchor only when the state actually flipped (still wrong — a long continued scroll left the anchor pinned at the flip point, so a later reveal measured against a stale anchor and became unreachable except near the top); v3 (correct) made the anchor track the running extreme point every tick. Each intermediate version had passing unit tests — because each one's tests only encoded the exact sequence the previous review comment had described, not the general invariant. For this class of function, write **sequence-based tests** up front (a multi-tick continued gesture in one direction; a deep-scroll-then-reverse; a jitter under the threshold) instead of one-shot single-call assertions, and every one of those three rounds also paid for a full rebuild-verify-push-CI cycle that a five-minute fuzz/sequence test would have skipped.
...
pnpm workspace, **Node 22, pnpm 10**. Run everything from the repo root. A fresh remote/CI checkout may have no `node_modules` at all (`jest`/`tsc` binaries missing) — run `pnpm install --frozen-lockfile` once before the jest-based lanes; the pure Node test lane (`node --test`, see below) only needs it if the specific test file imports a third-party package.
...
- **Pull requests: open as regular (non-draft) PRs, not drafts.** Once opened, proactively check CI status (`gh pr checks`) and address code reviews (including automated bot reviews).
- **Reading Bot Reviews & Feedback:** Automated tools like the Codex bot often post inline comments rather than top-level PR comments. If a bot signals a review but the main comment body is empty (or `gh pr view --comments` doesn't show actionable feedback), fetch the actual inline code review threads using `gh api repos/{owner}/{repo}/pulls/{number}/comments` or the `get_pull_request_comments` MCP tool to read and address the feedback before moving on. Once addressed and replied to, **always mark the review thread as resolved** (e.g., via the GraphQL API). Finally, always explicitly check for merge conflicts (e.g. `gh pr view {number} --json mergeable`) before requesting or performing a merge.
  - *Note on `gh api` usage:* Because agents operate across different environments (Linux/Bash and Windows/PowerShell), **always use file-based inputs** (e.g., `gh api graphql -F query=@query.graphql`) for complex payloads rather than relying on inline string escaping, which breaks unpredictably across OS boundaries.
- **Identify yourself first in every PR/issue comment** — lead with a short note that the comment is from Claude Code (an AI assistant), not a human collaborator, before the substantive content. Other AI agents work in this repo too (e.g. "Antigravity"), so this disambiguates who/what actually posted a given review or update.
- **Merging a `STASH-N` PR does NOT auto-close its Sentry issue.** In-app feedback (the `feedback-bridge` → Sentry flow) lands as Sentry issues with short-IDs `STASH-N` in org `self-463` / project `stash`; PRs that fix them are titled `STASH-N: …`. Sentry's GitHub integration only auto-resolves an issue when a **merged commit message contains `Fixes STASH-N`** — and since PRs here **squash-merge with the title as the commit message**, a `STASH-N:` *prefix* (not the `Fixes` keyword) never triggers it. So after merging, the issue stays `unresolved`: either put `Fixes STASH-N` in the PR title/squash body, or run `pnpm sentry:issue STASH-N --resolve`. To check whether an issue happened again in a release before resolving it, run `pnpm sentry:issue STASH-N --release 1.2.0-rc5`. The helper reads `SENTRY_AUTH_TOKEN` plus optional `SENTRY_ORG`/`SENTRY_PROJECT` from the shell, `.env.local`, or `SENTRY_ENV_FILE`. Do NOT rely on local docs (like AGENTS.md or build-history.md) to look up open/active Sentry issues as they only record historically resolved issues.
- CI can run on **either CircleCI or GitHub Actions** (`.circleci/config.yml` `ci` workflow and `.github/workflows/ci.yml`), running the same gate: `pnpm lint`, `pnpm typecheck`, `pnpm test`, **and `pnpm test:components`** — so the jest component lane is a real gate. A green `pnpm test` locally is **not** sufficient before pushing; run `pnpm test:components` too (it caught a data-loss-fix regression that the node lanes couldn't). CI migrated off GitHub Actions (quota exhausted) to CircleCI-only, then the Actions `ci.yml` was restored so a credit/quota outage on either provider still leaves a working gate — see `docs/development/ci-circleci.md`. Only one provider is meant to be active at a time: the GitHub Actions `checks` job is gated by the `CI_PROVIDER` repo variable (`github` = Actions runs, unset/anything else = CircleCI runs) so a credit outage doesn't mean burning both providers' resources on every push. Keep both configs' steps in sync if the gate changes. **Exception: the Android APK build stays on GitHub Actions** (see next bullet).
