...
- **Keep separate tasks on separate branches and PRs:** Always isolate different features or bug fixes on dedicated branches (using separate git worktrees if working on them concurrently). Never combine irrelevant tasks, bug fixes, or commits into the same branch or PR.
