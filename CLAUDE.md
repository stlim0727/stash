...
- CI runs **dual: CircleCI + GitHub Actions**, both on every push/PR (`.circleci/config.yml` `ci` workflow and `.github/workflows/ci.yml`), running the same gate in parallel: `pnpm lint`, `pnpm typecheck`, `pnpm test`, **and `pnpm test:components`** — so the jest component lane is a real gate. A green `pnpm test` locally is **not** sufficient before pushing; run `pnpm test:components` too (it caught a data-loss-fix regression that the node lanes couldn't). CI had migrated off GitHub Actions (quota exhausted) to CircleCI-only; the Actions `ci.yml` was restored alongside it once CircleCI ran out of credits, so a credit/quota outage on either provider still leaves a working gate — see `docs/development/ci-circleci.md`. Keep both configs' steps in sync if the gate changes. **Exception: the Android APK build stays on GitHub Actions** (see next bullet).
