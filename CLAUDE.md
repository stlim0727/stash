...
- `pnpm summary:users` — reports live user counts, bookmark totals, platform adoption, and device install-base (separating automated Play Store test bursts from organic users); needs `SUPABASE_SECRET_KEY` in `.env.local` or environment. Pass `--devices` or `--json` for targeted output.
...
- **Pull requests: open as regular (non-draft) PRs, not drafts.** Once opened or updated, actively monitor for CI failures, merge conflicts, and review comments (human and bot/AI agents). Address comments, resolve threads, and fix CI/conflicts per the PR workflow in `AGENTS.md`; stop monitoring after 5 minutes of idle time. **Never merge immediately upon CI green:** automated review bots (like Codex) typically post 2–4 minutes after PR creation/push. Merging prematurely bypasses review and lands unaddressed bot catches in trunk (e.g. #796).
- **Reading Bot Reviews & Feedback:** see the `resolve-pr-review-comments` skill for the fetch/reply/resolve procedure.
