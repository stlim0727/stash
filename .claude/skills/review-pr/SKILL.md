...
GitHub MCP tools (`mcp__github__pull_request_read`, `…__update_pull_request`,
`…__add_issue_comment`); fall back to `git` over the fetched refs when MCP is
unavailable.
...
- `pull_request_read` `method=get` — title, body, `state`, `draft`, `merged`,
  **`mergeable_state`** (`dirty` ⇒ conflicts with base), head/base refs,
  additions/deletions/changed_files.
- `method=get_files` — the actual diff. **Compare it against the title/body.**
...
- `method=get_check_runs` — CI status (`success` / `failure` / `skipped`).
- `method=get_reviews` and `method=get_review_comments` — existing human/bot
  feedback; don't repeat what's already been said.
...
- **Close as superseded/obsolete**: ALWAYS post an explanatory comment first
  (`add_issue_comment`) — say *why* (superseded by base, obsolete, duplicate),
...
- **Merge / changes-needed / general review**: ALWAYS post a comment on the PR
  containing your review findings and final verdict (using tools like `add_issue_comment`
  or the `gh pr comment` CLI when available). If no GitHub-write tools are exposed in
  the current session, report the exact comment text to the user and ask them to post it.
  Report the verdict to the user and let them decide; only merge when explicitly told to.
