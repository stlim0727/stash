...
description: >-
  ...
  bookmarks", "active users", or "how many anonymous vs registered users".
  Runs a fixed read-only query and formats a consistent table + insights
  (concentration, account types, empty/abandoned anonymous accounts, pending
  metadata, collection adoption) so the same request always yields the same
  shape of answer.
---
...
1. **Headline line:** total bookmarks, total users, split registered vs
   anonymous, and how many anonymous accounts are empty (0 bookmarks).
2. **Table of users WITH bookmarks** (drop the long tail of 0-bookmark rows from
   ...
   meta pending, last saved.
3. **Insights** — surface the product-relevant signals, not just the raw table:
