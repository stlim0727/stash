...
description: >-
  ...
  (create-or-update, never duplicate) into `.claude/skills/`, `AGENTS.md`, or
...
- **→ New/updated skill** — it's a repeatable *procedure* you'd re-run (a doer or
  a checker), multi-step, with a success criterion. Lives in `.claude/skills/`.
...
- **Look before you write.** Grep `.claude/skills/` and `AGENTS.md`/`CLAUDE.md`
...
- **New skill** → `.claude/skills/<name>/SKILL.md` in the house style: frontmatter
...
- If the work merits it, commit on the designated branch and open a PR the normal
  way; otherwise leave the edits staged for the user to review.
