...
- GitHub shows it as a **commit status** `ci/circleci: checks`, **not** a check-run → `mcp__github__get_job_logs` / check-run APIs return nothing useful.
...
```
node .claude/skills/circleci-logs/fetch-failure.mjs <workflow-id-or-url> [tailLines]
```
...
mcp__github__pull_request_read(method:"get_status", pullNumber:N)   # → statuses[].target_url has the workflow uuid
```
