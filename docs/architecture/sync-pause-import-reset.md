...
| **Pausing mid-sync (sync already started before pause was toggled)** | The in-flight run keeps going for its *current* chunk/entry, but every network loop re-checks `syncPausedRef` between steps: bulk bookmark creation, per-entry bookmark work, imported folder attachment, AI-enrichment restore, tag attachment/removal, and each page of a pull. The run stops as soon as the current request settles instead of draining the remaining outboxes or pagination. | An already-sent network request can't be un-sent; re-checking between steps is the closest thing to "stop now" that's actually safe. Completed upload chunks still persist their local queue updates before the run returns; an interrupted pull discards its partial snapshot and leaves the watermark unchanged. |
