...
| **Pausing mid-sync (sync already started before pause was toggled)** | The in-flight run keeps going for its *current* chunk/entry, but every network loop re-checks `syncPausedRef` between steps: bulk bookmark creation, per-entry bookmark work, imported folder attachment, AI-enrichment restore, tag attachment/removal, and pull. The run stops as soon as the current request settles instead of draining the remaining outboxes. | An already-sent network request can't be un-sent; re-checking between steps is the closest thing to "stop now" that's actually safe. Completed chunks still persist their local queue updates before the run returns. |
