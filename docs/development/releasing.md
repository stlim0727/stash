...
2. **Register an Android app**: Project → *Add app* → Android. Use package name
   **`com.keepory.app`** (matches `app.json`). A `google-services.json` is offered
...
| `FIREBASE_TEST_USERNAME_RESOURCE` | — | Android resource name of the username field (e.g. `com.keepory.app:id/email`) |
...
- iOS bundle identifier: `com.keepory.app`
- Android package: `com.keepory.app`
- iOS Share Extension app group: `group.com.keepory.app` (generated by the
  share-intent plugin).
