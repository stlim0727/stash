...
| Build | Date (UTC) | `main` SHA | What's new since last RC |
| ----- | ---------- | ---------- | ------------------------ |
| `v1.1.0-rc16` | 2026-07-07 | `30ff570` | Builds the latest trunk after rc15 (`694cc5a`): cadence-gates the SQLite reopen-churn Sentry alert so STASH-C reports stay useful (#398), teaches AI enrichment to reuse the user's existing tags to reduce vocabulary fragmentation (#395), extracts the suggestion-action workflow (#400), and adds the Tidy Up Phase 1/productization direction plus a tag-merge eval harness (#399). Build = `android-apk.yml` run #28838299650. |
| `v1.1.0-rc14` | 2026-07-06 | `bfe4f1d` | Data-loss fix: a new build that couldn't restore a real session was silently minting a throwaway **anonymous** user, and the next sync then deleted the real account's synced bookmarks locally (pull's `userChanged` deletion-skip guard only held for the first pull after the mint) and wiped the tag cache. Two layers (#375): (1) auth — a null secure-store read that evidences a previously-persisted real account now surfaces `session_expired` (keep local data, prompt re-sign-in) instead of minting anon, via a standalone real-account breadcrumb backfilled on read; (2) sync — an anonymous session never runs the remote-deletion diff (single-device by definition) and an anon empty-remote pull can't wipe a non-empty durable tag cache, returning the preserved snapshot so live UI keeps its tags. Server data was never affected. Build = `android-apk.yml` run #_(TBD)_. |
| `v1.1.0-rc2`–`rc13` | 2026-07-01 – 07-06 | _(various)_ | Not individually logged here — each build's `-rcN` is self-recorded on the rolling `dev` prerelease (name/body carry the label since #302), so the ledger row is narrative-only. rc13 (`7c07697`) was the last build before rc14 and carried the inbox top-cluster compaction (#363/#364/#367), the card Open-button fix (#369), and the bottom-`(tabs)` revert (#360). |
