...
- **Always start working on a new worktree when code work is needed.** Before making file modifications for a task, feature, or bugfix, create and switch to an isolated git worktree (e.g., `git worktree add <path> <branch>`) rather than editing directly in the primary workspace root. Keep the main working tree clean.
