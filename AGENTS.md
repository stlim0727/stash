...
## Next step: native build + on-device verification of M8, and verify M5–M7 against Supabase

**M8 needs a native build** — the share module cannot run in Expo Go. `pnpm --filter mobile android` / `ios` now run `expo run:android` / `expo run:ios` (dev client), which require Android Studio or Xcode (or use EAS Build). `expo prebuild` was validated in CI for both platforms: Android gets the `SEND`/`text/*` intent filter; iOS generates a Share Extension target with app group `group.<bundleId>` (set an Apple DEVELOPMENT_TEAM in Xcode for signing). To test: build a dev client, open another app, Share → Stash, confirm the link is saved to Inbox with a toast and no editor. Android is the lower-friction platform to verify first.

**M5–M7 need Supabase** — provide a project URL/key, enable anonymous sign-ins, apply `supabase/migrations/20260611000000_initial_schema.sql`, and verify anonymous session creation/restoration/refresh, RLS-scoped table access, API behavior, and end-to-end queue upload (save offline → entry syncs → bookmark gets remote ID and synced status). Then proceed to M9 (metadata enrichment placeholder) per the milestones doc.
