...
2. **Supabase (M5–M7)** — a project exists and `*.supabase.co` is on this environment's network allowlist (sessions started before that change are still blocked — the policy is fixed at container start). **Run `pnpm verify:supabase`** with `EXPO_PUBLIC_SUPABASE_URL=https://stzutoejnhzxzhjsjtsi.supabase.co` and `EXPO_PUBLIC_SUPABASE_ANON_KEY` (the publishable key; ask the user if not in the environment's env vars). The script exercises the app's real client/API code end to end — anonymous auth, refresh, restore, bookmark CRUD, dedupe, tags, AI enrichment, RLS isolation — cleans up after itself, and says clearly if anonymous sign-ins or the migration are missing on the Supabase side. Fix whatever it flags.
