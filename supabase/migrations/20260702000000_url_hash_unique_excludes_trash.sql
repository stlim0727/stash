...
-- occupied the unique slot for its url_hash. That made a trashed URL behave as
-- "already in stash": re-saving it collided with (or folded into) the trashed
-- row instead of coming back as a fresh active bookmark.
--
-- Redefine "active" the same way the app's inbox filter does — is_archived = false
-- AND deleted_at is null — so a trashed URL frees its slot and can be re-added.
