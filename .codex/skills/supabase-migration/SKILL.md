...
- Use `mcp__codex_apps__supabase._list_tables` to see the current structure and
  `_list_migrations` to see what's already applied. Do **not** assume the live
  DB matches the files — migrations have fallen through the cracks before (see
  the ordering trap).
...
1. If an **edge function** or a client insert/upsert references the new
   column/constraint, **apply the migration first**, then deploy the code. In
   Codex, use the exposed Supabase `_apply_migration` tool only for final DDL
   migration application; use `_execute_sql` for read-only inspection and safe
   iterative probes.
...
- **Apply:** `mcp__codex_apps__supabase._apply_migration` when you are ready to
  apply final DDL to the remote project. After applying, `_list_migrations` /
  `_get_advisors` to confirm it landed and flagged no new RLS/security
  advisory.
